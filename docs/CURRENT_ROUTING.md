...
- Latest resolved forest-plan review/inventory slice: all-R1 legacy direction and management-area inventory closeout plus source-set inventory authority/applicability gate hardening. The f70 manifest replay passed for 10/10 profiles with `component_count=5894`, `standard_count=1888`, zero duplicate IDs, and zero source-accuracy failures; `forest-plan-resolve` now requires the active source-set component inventory instead of silently falling back to the seed fixture; stale applicability authority universes fail against the current Forest Plan context inventory path/count; unresolved/unapplied standards and missing Forest Plan compliance matrices block reviewer-ready status.
...
- Forest Plan inventory route: active forest-plan component builds and reviews must use the manifest/source-set inventory path; management areas are first-class inventory scope, not profile stuffing or forest-specific handwritten rules; selected-profile reviews fail closed when the source-set inventory is missing, stale, or mismatched to the Forest Plan context, when applicable standards lack package/plan evidence, or when a resolved forest context lacks required Forest Plan compliance matrix rows/PDF evidence.
