...
- The post-V1 promotion suite is implemented at `config/promotion_suite_v1.json`. The latest
  ...
  and added ECID artifact checks, but the ECID slot remains blocked by Forest Plan component
  adjudication and the second expansion slot remains `package_fixture_missing`.
...
- The Sequence 2 ECID artifact pass generated and validated
  ...
  but compliance validation fails on `forest_plan_component_gate_reviewer_ready`; ECID identified
  `29` applicable Forest Plan standards, applied `7`, and has a `158`-item Forest Plan component
