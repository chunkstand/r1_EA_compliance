...
- graph boundary:
  ...
  trace, span, span-event, log-event, event-source, score, artifact,
  source-set, review, and failure-class nodes, with edges such as `CONTAINS`,
  `EMITTED`,
...
- live smoke:
  ...
  A second smoke with
  `--event-log-path /tmp/usfs-r1-explicit-event-log.jsonl` passed with `5,095`
