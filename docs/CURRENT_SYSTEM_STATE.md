...
- update:
  the third Milestone 4 all-step system-evaluation expansion slice is implemented for the
  current-promotion EA consistency decision-support contract. `ea-consistency-direct-eval` scores
  the generated decision-support report family and writes
  `source_library/reviews/v1-cg-ecid-compliance-review/decision_support/ea_consistency_decision_support_direct_eval_results.json`
  plus
  `source_library/reviews/v1-cg-ecid-compliance-review/decision_support/ea_consistency_decision_support_failure_intake_cases.json`.
  The tracked implementation lives in
  `src/usfs_r1_ea_sources/ea_consistency_decision_support_direct_eval.py`.
- contract:
  `ea_consistency_decision_support_direct_eval_results.json` carries schema
  `ea-consistency-decision-support-direct-eval-results-v1`, contract
  `ea-consistency-decision-support-direct-eval-v1`, scorer
  `ea-consistency-decision-support-direct-eval-deterministic-v1`, the exact review/source-set
  identity, source artifact hashes, and metric groups for identity/freshness, source-artifact
  replay, authority/count alignment, evidence traceability, artifact-hash alignment, and
  supervisor-boundary preservation. Failed groups generate replayable failure-intake cases.
...
- update:
  the first-class system-evaluation plan now has its first runtime
  applicability ratchet. `config/phase_eval_direct_eval_v1.json` names
  `applicability_validation` as a direct-eval-required phase only for
  `review_id="west-reservoir-67436"` and
  `source_set_id="source-set-f70ea11e04ae3d53"`. The phase is backed by
  `src/usfs_r1_ea_sources/phase_eval_direct_eval_applicability.py`.
- gate behavior:
  phase-eval requires
  `source_library/reviews/applicability_eval/applicability_eval_results.json`
  to carry schema `applicability-eval-results-v0`, contract
...
- scoped boundary:
  ...
  does not add a promotion gate, a global applicability ratchet, compliance
  gate-graph consumption, hosted/model-judge scoring, or live corpus
...
- update:
  the next source-owner oversized reduction slice is closed locally.
  `src/usfs_r1_ea_sources/phase_eval_direct_eval_source_set.py` remains the
  source-set direct-eval dispatcher at `721` lines. The new
  `src/usfs_r1_ea_sources/phase_eval_direct_eval_forest_plan.py` owns forest
  plan profile and component retrieval status/threshold handling at `345`
  lines.
- architecture effect:
  the architecture probe reports `546` code files, `3` code files above `800`,
  no Python or JS/TS import cycles, and no source module above the `20`-import
  fan-out gate. The live inventory now records `2` source owners and `1` test
  owner.
- boundary:
  this is a facade-preserving source split. It does not change direct-eval
  phase status semantics, generated artifact paths, phase-eval outputs, or
  local corpus state.
...
- update:
  the next source-owner oversized reduction slice is closed locally.
  `src/usfs_r1_ea_sources/extract_runtime.py` remains the extraction build
  orchestration facade at `565` lines. The new
  `src/usfs_r1_ea_sources/extract_runtime_reuse.py` owns extracted payload
  record/chunk assembly, current-output reuse, reuse-inventory reuse, and
  payload-cache helper logic at `301` lines.
- architecture effect:
  the architecture probe reports `543` code files, `7` code files above `800`,
  no Python or JS/TS import cycles, and no source module above the `20`-import
  fan-out gate. The live inventory now records `4` source owners and `3` test
  owners.
- boundary:
  this is a facade-preserving source split. It does not change extraction
  output schemas, parser behavior, generated artifact paths, or local corpus
  state.
...
- update:
  ...
  work preserved. The merged route now includes sidecar chunking, retrieval
  eval, graph/claim preview, graph/claim canonical adoption, sidecar rule-link
  readiness, compliance review adoption, phase-eval adoption, reviewer package
  lineage validation, and first-class generated observability/eval context
  graph coverage.
- boundary:
  no ignored production `source_library/` outputs are tracked by the merge.
  Knowledge-graph sidecar adoption, hosted query/API work, model-judge work,
  hosted scoring, and oversized-owner reduction remain future bounded packets.
...
- update:
  the generated observability/eval context graph includes conditional trace-event
  materialization, explicit local event-log capture, canonical CLI command-event
  capture, and a fail-closed f70 source-set phase-eval ratchet.
...
- contract:
  ...
  stop conditions.
- eval boundary:
  graph evals verify required node/edge coverage, edge resolution,
  trace-to-result-to-score paths, artifact provenance, source-KG exclusion,
  local source-of-record policy, event emission, event materialization, and
  command-event provenance. The graph is generated observability/eval evidence;
  it does not infer root causes, export Neo4j as source of record, capture raw
  prompt/source body content, or add promotion ratchets.
- live smoke:
  ...
  Source-set `phase-eval --source-set-id source-set-f70ea11e04ae3d53` passed
  `23/23` phases with
...
- update:
  `v1-cg-ecid-source-delta-review` remains a historical Custer Gallatin
  source-delta replay and component-eval coverage slot, not the active
  forest-specific example route. The tracked replay context now points at
  archived merged source set `source-set-8a4005c8a083af1a` and catalog dir
  `source_library/runs/r1-forest-plan-source-delta-capture-20260510-refresh-batches/merged_catalog_gate`.
  The tracked applicability adjudication metadata now matches the refreshed
  8a40 decision hash while preserving the same seven human-applicable
  adjudication items.
- refreshed local artifacts:
  retrieval direct eval passes `12/12` with `top_k=20`; claim direct eval
  passes `10/10`; base rule-claim direct eval passes `24/24`; compliance-review
  direct eval passes `5/5` on `source-set-8a4005c8a083af1a`; authority-universe,
  applicability validation, generated rule pack, generated rule-claim links,
  and the `36/36` forest-plan component eval are source-set aligned on
  `source-set-8a4005c8a083af1a`.
- current status:
  `phase-eval --review-id v1-cg-ecid-source-delta-review` now passes `22/22`
  phases with `reviewer_ready=true`, `blockers=[]`,
  ...
  replay has no declared review contract.
- route:
  do not describe ECID as a current-promotion blocker or as a remaining
  source-set split. No ECID source-delta residual is currently blocking the
  active forest-specific example route.
...
- update:
  ...
  is resolved locally through Milestone 4 promotion. Dead Laundry is now the
  governed primary Nez Perce-Clearwater example:
  ...
  `forest_unit_id="nez-perce-clearwater-nfs"`. `FOR-034` is resolved as the
  forest-specific example-package queue boundary.
- package boundary:
...
- governed boundary:
  `source_library/reviews/_intake/region1-example-nez-perce-clearwater-dead-laundry-57827/box_inventory.json`
  now records the governed narrowed replay boundary: `82` files across
  `13` folders, `234,693,626` expected bytes, explicit inclusion of
  specialist reports, draft/final EA, decision-process, and scoping
  materials, and explicit exclusion of `Analysis/EA references` plus
  `Decision/2023 Objection Materials Submitted`. The paired
  `box_import_manifest.json` records `82` downloaded files,
  `234,693,626` actual bytes, `document_count=82`, and `failure_count=0`.
- replay and base review:
  ...
  Base `ea-review` passes on the governed package with `82/82` extracted files,
  `2,991` package chunks, `package_failed_count=0`, and
  `reviewer_ready=true`.
- forest-plan and applicability:
  `forest-plan-resolve` now reports
  `scope_status="nez_perce_clearwater_nfs"`, `validation_passed=true`,
  `overlay_count=2`, `unresolved_mention_count=0`, and retrieval readiness
  passed. The tracked applicability adjudication at
  ...
  and generated rule-pack readiness. Generated rule-pack validation passes with
  `53` generated rules.
- reviewer stack and promotion:
  `compliance-review` writes `compliance_matrix.json`,
  `compliance_matrix.md`, and `compliance_matrix.pdf` with `53` findings
  (`33` `pass`, `19` `uncertain`, `1` `gap`), and validation is now
  reviewer-ready. The governed component adjudication at
  `config/forest_plan_component_adjudications/region1-example-nez-perce-clearwater-dead-laundry-57827.json`
  resolves `121/121` current queue items with disposition counts `53`
  `applicability_false_positive` and `68` `evidence_linking_miss`. V1 eval
  contract `config/v1_nez_perce_clearwater_dead_laundry_real_ea_eval.json`
  passes with contract status `reviewer_ready`; component eval contract
  ...
  passes `28/28` phases with `blockers=[]`,
  `declared_review_contract=true`, and
  `contract_backed_promotion_ready=true`.
- aggregate promotion and inherited component-coverage blocker resolved locally:
  ...
  the refreshed component eval passes `36/36` cases. Dead Laundry remains
  locally complete. The next inherited ECID issue, if reopened, is no longer
  component coverage, a source-set split, rule-claim direct eval, or stale
  compliance-review direct eval; those are resolved locally as described above.
...
- reviewer-stack replay:
  ...
  inherited `v1-cg-ecid-source-delta-review` slot was refreshed, with
  `covered_review_count=11/11`, `stale_identity_count=0`, and
  `unresolved_review_count=0`. Do not add Lacy Lemoosh package files to
...
- promotion boundary:
  ...
  The standalone aggregate is now green after the inherited
  `v1-cg-ecid-source-delta-review` slot was refreshed, with
  `covered_review_count=11/11`, `stale_identity_count=0`, and
  `unresolved_review_count=0`. Review
...
- component coverage:
  `config/forest_plan_component_eval_coverage_v1.json` includes the required
  Bitterroot Front component slot. The standalone aggregate is now green after
  the inherited `v1-cg-ecid-source-delta-review` slot was refreshed. The
  Bitterroot slot is covered, source-set aligned, and passing.
- queue boundary:
...
- component-inventory closure:
  ...
  `source-set-f70ea11e04ae3d53`. The review-local f70 manifest build under
  ignored
...
- component coverage boundary:
  ...
  The standalone aggregate `forest-plan-component-eval-coverage` command is
  now green after the inherited `ecid-source-delta-replay` /
  `v1-cg-ecid-source-delta-review` slot was refreshed; current aggregate
  counts are `covered_review_count=11/11`, `stale_identity_count=0`, and
  `unresolved_review_count=0`.
...
- update:
  `docs/FIRST_CLASS_EVAL_TRACE_IMPLEMENTATION_MILESTONE_PLAN.md` Milestone 2 is
  resolved locally. The system now has a local SQLite-backed first-class
  eval/trace store build command over the green inventory output.
- owner surfaces:
  `src/usfs_r1_ea_sources/eval_trace_store.py`,
  `src/usfs_r1_ea_sources/cli_eval.py`, `tests/test_eval_trace_store.py`,
  `tests/test_cli_eval.py`, `docs/OUTPUT_SCHEMAS.md`,
  `docs/EVALUATION_COVERAGE_REGISTER.md`, `docs/ARCHITECTURE.md`, and
  `docs/architecture_contract.toml`.
- store truth:
  `eval-trace-store-build` reads an `eval-trace-inventory-results-v1` JSON file
  and rebuilds the generated `system_eval_trace.sqlite` store. The canonical
  tables are `system_eval_runs`, `system_eval_cases`,
  `system_eval_case_results`, `system_eval_scores`, `trace_runs`, and
  `trace_spans`. Rows preserve origin artifact refs, artifact hashes,
  contract ID/version, source-set ID, review ID, catalog refs, replay-context
  refs, source-record IDs when present, scorer-version metadata, threshold
  metadata, and typed failure categories.
- West Reservoir seed build:
  `eval-trace-store-build --inventory-path
  /tmp/usfs-r1-eval-trace-inventory.json --sqlite-path
  /tmp/usfs-r1-system-eval-trace.sqlite --summary-path
  /tmp/usfs-r1-system-eval-trace-summary.json` passed with `18` rows in each
...
- West Reservoir seed run:
  ...
  rows.
- ratchet/export state:
  global fail-closed eval-trace ratchets are still not enabled. Inventory still
  reports `export_readiness.reason="sqlite_store_not_built"` because export
  readiness is owned by later store/export milestones; the newer Milestone 2
  section above now records the implemented local SQLite store.
- next implementation route:
  superseded by the newer Milestone 2 section above; the next route is
  Milestone 3 canonical/OpenInference export. Do not add phase/promotion gates
  or trace-to-case promotion before export is implemented and verified.
...
- update:
  ...
  contract. Later sections above now record the implemented inventory CLI and
  local SQLite store.
- owner surfaces:
  `docs/FIRST_CLASS_EVAL_TRACE_CONTRACT.md`,
  `config/eval_trace_inventory_contract_v1.json`,
...
- ratchet state:
  ...
  read-only CLI and stored by the Milestone 2 local SQLite build, but no
  export or ratchet is enabled yet.
- next implementation route:
  superseded by the newer Milestone 2 section above; the next route is
  Milestone 3 canonical/OpenInference export. Do not build phase/promotion
  gates before export is green.
...
- residual blocker:
  `forest-plan-component-eval-coverage` is now green after the inherited
  `ecid-source-delta-replay` / `v1-cg-ecid-source-delta-review` slot was
  refreshed. The West Reservoir slot still passes and source-set aligns;
  aggregate counts are `covered_review_count=11/11`,
  `stale_identity_count=0`, and `unresolved_review_count=0`.
- next implementation route:
...
- component aggregate truth:
  `forest-plan-component-eval-coverage` is now green after the inherited
  `ecid-source-delta-replay` / `v1-cg-ecid-source-delta-review` slot was
  refreshed. West Reservoir still passes and source-set aligns. Aggregate
  counts are `covered_review_count=11/11`, `stale_identity_count=0`, and
  `unresolved_review_count=0`.
- stop condition:
...
- component coverage truth:
  ...
  passing. Later packets resolved the pre-existing ECID source-delta and West
  Reservoir component-coverage residuals; current aggregate counts are
  `covered_review_count=11/11`, `stale_identity_count=0`, and
  `unresolved_review_count=0`.
- phase truth:
...
- component-coverage boundary:
  ...
  on `source-set-f70ea11e04ae3d53`, and later packets resolved the non-Lolo
  source-delta and West Reservoir aggregate residuals with
  `covered_review_count=11/11`, `stale_identity_count=0`, and
  `unresolved_review_count=0`.
- verification:
...
- aggregate component-coverage truth:
  ...
  aligned to `source-set-f70ea11e04ae3d53` and passes, and later packets
  resolved the non-Lolo aggregate residuals:
  `covered_review_count=11/11`, `stale_identity_count=0`, and
  `unresolved_review_count=0`.
- next routing:
