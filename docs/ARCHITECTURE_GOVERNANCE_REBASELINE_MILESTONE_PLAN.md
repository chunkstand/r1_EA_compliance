...
the current oversized-file inventory is empty after the zero-over-`800` closeout
...
### Live architecture evidence on 2026-06-01
...
- Fresh architecture probe:
  ...
  reports `548` code files, `0` code files above `800` lines, no Python or JS/TS import cycles,
  and no source module above the `20`-import fan-out gate.
- `config/architecture_large_file_inventory_v1.json` records an empty live backlog:
  `0` source-owner families and `0` test-owner families.
- `src/usfs_r1_ea_sources/eval_trace_inventory.py` is resolved as the final oversized source
  owner: eval trace inventory runtime remains in the original file at `793` lines, and the
  inventory result/spec dataclasses now live in
  `src/usfs_r1_ea_sources/eval_trace_inventory_models.py` at `19` lines.
- `src/usfs_r1_ea_sources/applicability_candidate_assembly.py` is resolved as an oversized source
  owner: candidate assembly remains in the original file at `778` lines, and shared rule/component
  package fact-type helpers now live in
  `src/usfs_r1_ea_sources/applicability_contract_support.py` at `174` lines.
- `tests/test_catalog.py` is resolved as an oversized test owner:
  catalog behavior assertions remain in the original file at `647` lines, and shared
  download/batch/catalog role fixture helpers now live in `tests/support/catalog_fixtures.py` at
  `187` lines.
- `src/usfs_r1_ea_sources/phase_eval_direct_eval_source_set.py` is resolved as an oversized source
  owner: the source-set direct-eval dispatcher is `721` lines, and forest-plan profile/component
  retrieval status and threshold handling now live in
  `src/usfs_r1_ea_sources/phase_eval_direct_eval_forest_plan.py` at `345` lines.
...
- `tests/test_forest_plan_profiles.py` is resolved as an oversized test owner:
  ...
  `tests/test_forest_plan_profile_contexts.py` at `256` lines.
- `src/usfs_r1_ea_sources/ea_consistency_decision_support_inputs.py` is resolved as an oversized
  source owner: the input loading/validation facade is `390` lines, and validation helper checks
  now live in `src/usfs_r1_ea_sources/ea_consistency_decision_support_input_validation.py` at
  `530` lines.
- `tests/test_promotion_suite_full_canonical.py` is resolved as an oversized test owner:
  committed manifest full-canonical assertions remain in the original file at `534` lines, and
  runtime full-canonical readiness/path cases now live in
  `tests/test_promotion_suite_full_canonical_runtime.py` at `395` lines.
- `src/usfs_r1_ea_sources/eval_trace_store.py` is resolved as an oversized source owner:
  the eval trace store build facade is `733` lines, and SQLite schema/row/count helpers now live in
  `src/usfs_r1_ea_sources/eval_trace_store_sqlite.py` at `204` lines.
- `src/usfs_r1_ea_sources/source_register_proving.py` is resolved as an oversized source owner:
  the proving-slice orchestration facade is `729` lines, and synthetic download/context I/O helpers
  now live in `src/usfs_r1_ea_sources/source_register_proving_io.py` at `220` lines.
- `tests/test_cli_eval.py` is resolved as an oversized test owner: handler propagation tests remain
  in the original file at `585` lines, and parser coverage now lives in
  `tests/test_cli_eval_parsers.py` at `355` lines.
- `src/usfs_r1_ea_sources/claim_extraction_eval.py` is resolved as an oversized source owner:
  the public facade is `326` lines, and query/scoring/readiness/eval-contract helpers now live in
  `src/usfs_r1_ea_sources/claim_extraction_eval_support.py` at `668` lines.
