...
- runtime owner:
  ...
  review-coverage slots, source-set binding, family pass state, and separate
  reference-canary status; `docs/architecture_contract.toml` now declares that
...
queue family in
`docs/FULL_CANONICAL_DIRECT_FILE_CAPTURE_QUEUE_RESOLUTION_MILESTONE_PLAN.md`
without pretending that the `FINAL-Q-NPC-001` Box planning-record share is a
clean single-family promotion.

- outcome label:
  `reduced locally`; `FINAL-Q-NPC-001` is now an explicit blocker family, the
...
- workbook and queue truth:
  ...
  `unresolved_current_or_project_applicable_count=34`, and governed
  historical rows `FPS-380` and `SUP-007`
- new blocker family:
  `FINAL-Q-NPC-001` now routes to
  `docs/NEZ_PERCE_CLEARWATER_PLANNING_RECORD_BLOCKER_MILESTONE_PLAN.md`
  because the live Box share is a multi-page NPC planning-record library whose
  visible top-level folders already mix core plan documents with high-volume
  FEIS-reference, objection-reference, consultation, amendment, infrastructure,
  and misc-support surfaces
...
- next routing:
  ...
  `docs/FULL_CANONICAL_DIRECT_FILE_CAPTURE_QUEUE_RESOLUTION_MILESTONE_PLAN.md`
  for the remaining export-backed family `LEX-Q-001`, while the active blocker
  packets are now
...
queue family in
`docs/FULL_CANONICAL_DIRECT_FILE_CAPTURE_QUEUE_RESOLUTION_MILESTONE_PLAN.md`
without pretending that the `WILD-ESA-Q001` public folder is a clean
single-family promotion.

- outcome label:
  `reduced locally`; `WILD-ESA-Q001` is now an explicit blocker family, the
  SCC structured-export and Flathead blocker slices remain in place, and the
  next routed slice stays on the remaining generic export-backed families
- routed packet:
  `docs/FULL_CANONICAL_DIRECT_FILE_CAPTURE_QUEUE_RESOLUTION_MILESTONE_PLAN.md`
- workbook and queue truth:
  `source-register-diff` remains
  ...
  `unresolved_current_or_project_applicable_count=36`, and governed
  historical rows `FPS-380` and `SUP-007`
- new blocker family:
  `WILD-ESA-Q001` now routes to
  `docs/NCDE_GRIZZLY_BEAR_AMENDMENT_EXPORT_BLOCKER_MILESTONE_PLAN.md`
  because the live public export roster mixes overlapping Flathead plan/FEIS
  records with distinct multi-forest NCDE amendment documents and still-missing
  Flathead appendix-map surfaces
- active catalog truth:
  the live full-canonical catalog remains
  `source-set-4fb59e9eb43045cb` with `source_count=647`,
  `artifact_count=635`,
...
- next routing:
  ...
  `FINAL-Q-NPC-001`, and `LEX-Q-001`, while the active blocker packets are now
  `docs/FLATHEAD_READING_ROOM_FILE_SET_BLOCKER_MILESTONE_PLAN.md`,
...
This implementation slice narrows the next Milestone `3` queue family in
`docs/FULL_CANONICAL_DIRECT_FILE_CAPTURE_QUEUE_RESOLUTION_MILESTONE_PLAN.md`
without pretending that the Flathead reading-room family is already fully
captured.

- outcome label:
  `reduced locally`; `FINAL-Q-FLAT-001` is now an explicit blocker family,
  the SCC structured-export slice remains resolved, and the next routed slice
...
- workbook and queue truth:
  ...
  `unresolved_current_or_project_applicable_count=37`, and governed
  historical rows `FPS-380` and `SUP-007`
- new blocker family:
  `FINAL-Q-FLAT-001` now routes to
  `docs/FLATHEAD_READING_ROOM_FILE_SET_BLOCKER_MILESTONE_PLAN.md` because the
  current canonical workbook only partially materializes the Flathead public
  reading-room file set; remaining BA/BO, monitoring/BMER, administrative
  change, cover-letter, response-to-comments, forest-plan-amendments, and
  appendix-map documents still need governed successor capture or explicit
  exclusion
- active catalog truth:
  the live full-canonical catalog remains
  `source-set-4fb59e9eb43045cb` with `source_count=647`,
...
- next routing:
  stay on Milestone `3` in
  `docs/FULL_CANONICAL_DIRECT_FILE_CAPTURE_QUEUE_RESOLUTION_MILESTONE_PLAN.md`
  for the remaining export-backed families `WILD-ESA-Q001`,
  `FINAL-Q-LOLO-001`, `FINAL-Q-NPC-001`, and `LEX-Q-001`, while the active
  blocker packets are now
  `docs/FLATHEAD_READING_ROOM_FILE_SET_BLOCKER_MILESTONE_PLAN.md`
  and
  `docs/PROJECT_SPECIFIC_PUBLIC_PRIVATE_SOURCE_BOUNDARY_BLOCKER_MILESTONE_PLAN.md`
- closeout commit:
...
- downstream revalidation truth:
  ...
  is green with `585` admitted active-current rows, `0` blocked rows, and
  `53` explicit archive/currentness rows;
  `authority-currentness --source-set-id source-set-3f7d4578cafb0704` is
  green with `authority_family_count=456` and
...
- residual note:
  ad hoc full-canonical
  `phase-eval --source-set-id source-set-3f7d4578cafb0704` still lacks
  extraction, retrieval, claim-extraction, and rule-claim direct-eval
  coverage on the strengthened source set and is not part of this milestone's
  acceptance gate
- next routing:
...
- blocked roster:
  ...
  blocked active-current row:
  `USFS-026` (`FSH 2509.18`); `USFS-018` and `USFS-024` now admit through
  live current official Forest Service static-file PDFs, while `USFS-026`
...
- next routing:
  ...
  the remaining `1` directives-wrapper direct-document blocker through
  governed promotion or explicit exclusion
- stale-reference audit:
  the immediately following manual-wrapper slice is preserved append-only
  history only; this manual-redirect slice supersedes the earlier `579/582`
  plus `3`-blocker checkpoint for live routing
- verification:
...
- outcome label:
  `reduced`; a handbook-wrapper adapter now admits `9` legacy USFS handbook
...
- blocked roster:
  ...
  `USFS-039`; the remaining family is now `12` FSM/manual wrapper pages plus
  `USFS-026`, whose live National Directives entry exposes only a transmittal
  link rather than a current contents page
- next routing:
  continue with Milestone `2` in
  `docs/FULL_CANONICAL_SOURCE_TRUTH_REBASELINE_MILESTONE_PLAN.md` to resolve
  the remaining `13` directives-wrapper direct-document blockers through
  governed promotion, manual-page direct-document routing, or explicit
  exclusion
- stale-reference audit:
  the immediately following Milestone `2` reduced section is preserved
  append-only history only; this handbook-wrapper slice supersedes the earlier
  `560/582` plus `22`-blocker checkpoint for live routing
- verification:
  `PYTHONPATH=src .venv/bin/python -m pytest tests/test_adapters_report.py tests/test_extraction_accuracy.py tests/test_retrieval_validation.py tests/test_architecture_contract.py -q`,
...
legacy/current source-record drift as the main live owner. The packet is still
red, now on five unmapped authorities plus narrower source-claim-link
expectation drift.

- outcome label:
  Milestone `2` reduced again; the closeout for this reduced slice is complete;
  the packet remains active on the next Milestone `2` owner family
- routed packet:
  `docs/FULL_CANONICAL_COMPLIANCE_GOLD_REBASELINE_MILESTONE_PLAN.md`
- runtime repair:
  `config/compliance_source_record_reconciliation_v1.json` now governs
  legacy-to-canonical authority source-record mapping; the shared helper in
  `src/usfs_r1_ea_sources/records.py` now widens retrieval filters,
  rule-claim filter validation, and eval source-record comparisons.
  `src/usfs_r1_ea_sources/retrieval.py`,
  `src/usfs_r1_ea_sources/ea_review.py`,
  `src/usfs_r1_ea_sources/rule_claim_binding_runtime.py`,
  `src/usfs_r1_ea_sources/rule_claim_binding_validation.py`, and
  `src/usfs_r1_ea_sources/compliance_review_eval.py` now consume that governed
  reconciliation surface, and compliance-review eval now treats every
  `*_diagnostic` applicability gate mode as diagnostic for expectation
  defaults and trace-coverage scoring
- fresh isolated replay truth:
  ...
  still finishes red with `passed_case_count=0/14`, but the old all-`uncertain`
  collapse is gone. The replay now records
  `authority_trace_coverage_rate=1.0`,
  aggregate finding statuses `226 pass / 166 gap / 268 uncertain`, and
  `gold-all-authorities-supported` now scores `39` live `pass` findings and
  `20` `uncertain` findings
- residual owner truth:
  source-record and source-document-role mismatches are now limited to
  `apa_final_agency_action`,
  `directives_notice_comment_36cfr_216`,
  `musuya_multiple_use_sustained_yield`,
  `organic_act_16usc_475`, and
  ...
  because those five authorities still have no current canonical row on
  `source-set-f775524ab233ff27`.
  Source-claim-link mismatches are also narrower:
  ...
  positive links
- bounded aggregate truth:
  `source_library/reviews/gold_coverage_eval_seq52_fix4/gold_coverage_eval_results.json`
  remains red only because `compliance_gold_failed=1`; it still records
  `required_theme_count=7`, `passed_theme_count=7`,
  `distinct_forest_count=2`, `distinct_package_style_count=3`,
  `reviewer_ready_review_count=2`, and `typed_blocked_review_count=1` with no
  threshold failures
- next routing:
  continue the same packet on Milestone `2`, now focused on the five unmapped
  authorities and the narrowed review-time source-claim-link expectation drift
  rather than the earlier applicability abort or broad source-ID mismatch
- verification:
  ...
  `PYTHONPATH=src python -m compileall src`,
  the fresh `compliance-gold-eval` replay above,
  the bounded `gold_coverage_eval_seq52_fix4` replay, and `git diff --check`
...
`tests/support/compliance_review_fixtures.py` hotspot split, records the new compliance-review
support owners in the routed packet, and keeps the broader Milestone 8 test-hotspot packet moving
on the next oversized test owner from the live architecture probe.

- outcome label:
  `reduced` for Milestone 8 sequence 45; the broader Milestone 8 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- compliance-review-core-support-owner closeout:
  `tests/support/compliance_review_fixtures.py` now owns the source-library bootstrap, package and
  generated-review gate builders, grouped conditional rule-pack fixture, shared JSON writers, and
  report/validation lookup helpers that remain on the core compliance-review lane
- compliance-review-eval-support-owner closeout:
  ...
  assertion coverage that previously remained inline in
  `tests/support/compliance_review_fixtures.py`
- compliance-review-import-routing closeout:
...
- compliance-review-boundary-owner closeout:
  `tests/test_compliance_review_test_boundary.py` now records the split support-owner budgets for
  `tests/support/compliance_review_fixtures.py`,
  `tests/support/compliance_review_eval_fixtures.py`, and the now-sub-`800` eval/phase-eval owner
  files
- review-phase-contract closeout:
...
- direct contract coverage:
  ...
  `tests/test_compliance_gold_eval.py`,
  `tests/test_compliance_phase_eval.py`, and
  `tests/test_compliance_review_test_boundary.py` still verify compliance-review core routing,
  grouped conditional rule-pack validation, eval/coverage/gold-eval contracts, phase-eval
  review-phase behavior, and owner-boundary enforcement after the support split
- architecture closeout:
  `tests/test_architecture_quality.py` tightens the oversized-file baseline from `28` to `27`
  after the `tests/support/compliance_review_fixtures.py` hotspot is reduced below the
  `800`-line gate
- live probe evidence:
  the fresh architecture probe reports `338` code files, `27` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `tests/support/compliance_review_fixtures.py=694`,
  `tests/support/compliance_review_eval_fixtures.py=430`,
  `tests/test_compliance_review_test_boundary.py=130`, and
  `tests/support/compliance_component_fixtures.py=872`, no remaining modules above the
  `20`-import fan-out gate, and no Python or JS/TS import cycles
- residual system state:
  runtime behavior is unchanged in this slice; the verification focus stayed on compliance-review
  support-owner routing, review-phase contract alignment, and architecture gates because only
  test/support surfaces and routed docs changed
- next routing:
...
`tests/test_project_sow_package.py` hotspot split, records the new Project SOW test owners plus
shared fixture helpers in the routed packet, refreshes the live Project SOW verification docs, and
keeps the broader Milestone 8 test-hotspot packet moving on the next oversized test owner from the
live architecture probe.

- outcome label:
  `reduced` for Milestone 8 sequence 38; the broader Milestone 8 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- project-sow-fixture-support closeout:
  ...
  helpers that previously remained inline in `tests/test_project_sow_package.py`
- project-sow-adjudication-owner closeout:
  `tests/test_project_sow_adjudication.py` now owns the adjudication template, adjudication eval,
  adjudication apply, and EA package handoff coverage that previously remained inline in
  `tests/test_project_sow_package.py`
- project-sow-intake-owner closeout:
  `tests/test_project_sow_intake.py` now owns the intake schema, intake draft,
  reviewer-confirmed draft replay, validation-only acceptance, and invalid-intake coverage that
  previously remained inline in `tests/test_project_sow_package.py`
- project-sow-validation-owner closeout:
  `tests/test_project_sow_package_validation.py` now owns the Project SOW graph and package
  failure-path coverage that previously remained inline in `tests/test_project_sow_package.py`
- project-sow-boundary-owner closeout:
...
- project-sow-doc closeout:
  `docs/PROJECT_SOW_OPERATIONALIZATION_MILESTONE_PLAN.md`,
  `docs/PROJECT_SOW_OPERATIONALIZATION_ACCEPTANCE_MATRIX.md`, and
  `docs/PROJECT_SOW_REQUIREMENTS_PACKAGE_MILESTONE_PLAN.md` now point their live verification
  references at the split Project SOW owner family instead of the retired monolith
- direct contract coverage:
  `tests/test_project_sow_package.py`, `tests/test_project_sow_adjudication.py`,
  `tests/test_project_sow_intake.py`, `tests/test_project_sow_package_validation.py`, and
  `tests/test_project_sow_package_test_boundary.py` still verify requirements-package generation,
  proving eval, operational-gate release checks, adjudication replay, EA handoff, intake draft and
  validation, graph failure paths, and owner-boundary enforcement after the split
- architecture closeout:
  `tests/test_architecture_quality.py` tightens the oversized-file baseline from `35` to `34`
  after the `tests/test_project_sow_package.py` hotspot is reduced below the `800`-line gate
- live probe evidence:
  ...
  `20`-import fan-out gate, and no Python or JS/TS import cycles
- residual system state:
  runtime behavior is unchanged in this slice; the verification focus stayed on Project SOW test
  owner routing, live verification doc references, and architecture gates because only test-owner
  surfaces and routed docs changed
- next routing:
  continue the same umbrella packet inside Milestone 8 on `tests/test_forest_plan_components.py`
- verification:
...
records the new V1 eval modules in the architecture surfaces, and lets the broader Milestone 7
eval/promotion packet advance to `promotion_suite.py` instead of staying pinned on the V1 review
eval owner.

- outcome label:
  `reduced` for Milestone 7 sequence 27; the broader Milestone 7 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- v1-eval-support-owner closeout:
  `src/usfs_r1_ea_sources/v1_ea_eval_support.py` now owns eval-contract loading, review artifact
  IO, review-identity matching, contract validation, and shared normalization/JSON helpers that
  previously remained in `v1_ea_eval.py`
- v1-eval-rule-owner closeout:
  `src/usfs_r1_ea_sources/v1_ea_eval_rule_expectations.py` now owns section detection, baseline
  authority alignment, rule-review expectation scoring, and applicability-decision
  surrogate/evidence helpers that previously remained in `v1_ea_eval.py`
- v1-eval-conditional-owner closeout:
  `src/usfs_r1_ea_sources/v1_ea_eval_conditional.py` now owns conditional expectation scoring and
  conditional adjudication-policy reporting that previously remained in `v1_ea_eval.py`
- v1-eval-forest-plan-owner closeout:
  `src/usfs_r1_ea_sources/v1_ea_eval_forest_plan.py` now owns forest-plan expectation scoring,
  forest-plan authority bridge rows, and reviewer-resolution/component-adjudication helpers that
  previously remained in `v1_ea_eval.py`
- v1-eval-summary-owner closeout:
  `src/usfs_r1_ea_sources/v1_ea_eval_summary.py` now owns check synthesis, metrics, failure
  aggregation, lane-state evaluation, and contract-status calculation that previously remained in
  `v1_ea_eval.py`
- facade-preserving routing:
  `src/usfs_r1_ea_sources/v1_ea_eval.py` keeps the public `run_v1_ea_review_eval(...)` surface
  and now owns only orchestration, payload assembly, and writeback without changing callers
- direct contract coverage:
  `tests/test_v1_ea_eval.py`, `tests/test_real_package_review_coverage_eval.py`, and
  `tests/test_gold_coverage_eval.py` still verify the public V1 review eval surface and its
  manifest-driven callers end to end after the split
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `v1_ea_eval_support.py`,
  `v1_ea_eval_rule_expectations.py`, `v1_ea_eval_conditional.py`,
  `v1_ea_eval_forest_plan.py`, and `v1_ea_eval_summary.py` explicitly in the eval container, and
  `docs/architecture_contract.toml` assigns all five new modules to the `eval` layer
- live probe evidence:
  the fresh architecture probe reports `269` code files, `43` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `v1_ea_eval.py` reduced to `298` lines from the pre-sequence `2662`-line baseline, new modules
  `v1_ea_eval_support.py=521`, `v1_ea_eval_rule_expectations.py=498`,
  `v1_ea_eval_conditional.py=246`, `v1_ea_eval_forest_plan.py=502`,
  `v1_ea_eval_summary.py=712`, one allowed fan-out hotspot at `cli_derived=22`, and no Python or
  JS/TS import cycles
- residual system state:
  ...
  new regression from the V1 eval owner split
- next routing:
  continue Milestone 7 inside the same umbrella packet on `promotion_suite.py`; do not advance to
  `upstream_evaluation.py` or the CLI orchestration owners until the promotion-suite seam is
  narrowed
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_v1_ea_eval.py tests/test_real_package_review_coverage_eval.py tests/test_gold_coverage_eval.py tests/test_phase_eval_direct_eval_contracts.py -q`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
...
- outcome label:
  `reduced` for Milestone 6 sequence 18; the broader Milestone 6 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- rule-claim-validation-owner closeout:
  `src/usfs_r1_ea_sources/rule_claim_binding_validation.py` now owns rule-claim artifact
  validation, rule-pack/claim readiness checks, deterministic link/gap identity validation,
  provenance/filter/score/rank checks, and sqlite validation support that previously remained in
  `rule_claim_binding.py`
- facade-preserving routing:
  `src/usfs_r1_ea_sources/rule_claim_binding.py` keeps the public `rule-claim-link`,
  `rule-claim-validate`, and `rule-claim-eval` workflow surface, now holds the remaining
  rule-matching and build core, and delegates validation support to the new owner module without
  changing callers
- direct contract coverage:
  ...
  workflow end to end
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `rule_claim_binding_validation.py` explicitly in the
  evidence-and-claims container, and `docs/architecture_contract.toml` assigns it to the `claims`
  layer
- live probe evidence:
  the fresh architecture probe reports `234` code files, `52` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `rule_claim_binding.py` reduced to `849` lines from the post-sequence-17 `1360`-line baseline,
  new module `rule_claim_binding_validation.py=592`, `tests/test_rule_claim_binding.py` reduced to
  `600`, new test seam `tests/test_rule_claim_binding_validation.py=163`, one fan-out hotspot at
  `cli_derived=22`, and no Python or JS/TS import cycles
- residual risk:
  `rule_claim_binding.py` still remains just above the repo gate and now owns the remaining
  rule-matching, scoring, record-assembly, and sqlite-write core, so the broader claims/evidence
  hotspot family is still open beyond the validation boundary
- next routing:
  continue Milestone 6 inside the same umbrella packet on the remaining `rule_claim_binding.py`
  build core, then `evidence_graph.py`, `package_fact_graph.py`, `applicability_retrieval.py`,
  `applicability_rule_pack.py`, and `applicability_eval.py`; do not advance to Milestone 7 yet
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_rule_claim_binding.py tests/test_rule_claim_binding_eval.py tests/test_rule_claim_binding_validation.py -q`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
...
- outcome label:
  `reduced` for Milestone 6 sequence 16; the broader Milestone 6 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- claim-runtime-owner closeout:
  `src/usfs_r1_ea_sources/claim_extraction_runtime.py` now owns deterministic claim-pattern
  definitions, sentence/window handling, claim record assembly, IDs/hashes, and extraction metrics
  that previously remained in `claim_extraction.py`
- facade-preserving routing:
  `src/usfs_r1_ea_sources/claim_extraction.py` keeps the public `claim-extract`,
  `claim-validate`, and `claim-eval` workflow surface, now holds orchestration support, and
  delegates the extraction runtime to the new owner module without changing downstream callers
- direct contract coverage:
...
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `claim_extraction_runtime.py` explicitly in the
  evidence-and-claims container, and `docs/architecture_contract.toml` assigns it to the `claims`
  layer
- live probe evidence:
  the fresh architecture probe reports `230` code files, `52` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `claim_extraction.py` reduced to `458` lines from the post-sequence-15 `783`-line baseline,
  new module `claim_extraction_runtime.py=342`, `claim_extraction_validation.py=614`,
  `tests/test_claim_extraction.py` reduced to `712`, one fan-out hotspot at `cli_derived=22`, and
  no Python or JS/TS import cycles
- residual risk:
  the broader claims/evidence hotspot family remains active even though the claim owner family is
  now reduced to explicit runtime, validation, eval, and graph seams
- next routing:
  continue Milestone 6 inside the same umbrella packet on `rule_claim_binding.py`, then
  `evidence_graph.py`, `package_fact_graph.py`, `applicability_retrieval.py`,
  `applicability_rule_pack.py`, and `applicability_eval.py`; do not advance to Milestone 7 yet
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_claim_extraction.py tests/test_claim_extraction_runtime.py tests/test_claim_extraction_eval.py tests/test_claim_extraction_validation.py -q`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_rule_claim_binding.py tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
  and `git diff --check`
...
in the architecture surfaces, and keeps the umbrella packet routed inside Milestone 6 instead of
pretending the broader claims/evidence family is closed.

- outcome label:
  `reduced` for Milestone 6 sequence 14; the broader Milestone 6 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- claim-eval-owner closeout:
  `src/usfs_r1_ea_sources/claim_extraction_eval.py` now owns deterministic claim eval scoring,
  legacy/current eval contract loading, coverage and metric-threshold checks, and claim-readiness
  revalidation that previously remained in `claim_extraction.py`
- facade-preserving routing:
  `src/usfs_r1_ea_sources/claim_extraction.py` keeps the public `claim-extract`,
  `claim-validate`, and `claim-eval` workflow surface, now holds the remaining claim extraction
  and validation core, and re-exports `_load_validated_claims_for_eval(...)` for
  `rule_claim_binding.py`
- direct contract coverage:
...
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `claim_extraction_eval.py` explicitly in the evidence-and-claims
  container, and `docs/architecture_contract.toml` assigns it to the `claims` layer
- live probe evidence:
  the fresh architecture probe reports `226` code files, `54` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `claim_extraction.py` reduced to `1328` lines from the post-sequence-13 `2084`-line baseline,
  new module `claim_extraction_eval.py=800`, one fan-out hotspot at `cli_derived=22`, and no
  Python or JS/TS import cycles
- residual risk:
  `claim_extraction.py` still remains a large mixed-owner file because the remaining extraction and
  validation core is still coupled in the public facade, and `tests/test_claim_extraction.py`
  remains above the `800`-line gate for the later test-boundary milestone
- next routing:
  continue Milestone 6 inside the same umbrella packet on the remaining claim extraction and
  validation core in `claim_extraction.py`, then `rule_claim_binding.py`,
  `evidence_graph.py`, `package_fact_graph.py`, `applicability_retrieval.py`,
  `applicability_rule_pack.py`, and `applicability_eval.py`; do not advance to Milestone 7 yet
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_claim_extraction.py tests/test_claim_extraction_eval.py -q`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
...
This thirteenth Milestone 6 slice closes the first `claim_extraction.py` graph-owner seam,
records the new owner module in the architecture surfaces, and keeps the umbrella packet routed
inside Milestone 6 instead of pretending the broader claims/evidence family is closed.
...
- facade-preserving routing:
  `src/usfs_r1_ea_sources/claim_extraction.py` keeps the public `claim-extract`,
  `claim-validate`, and `claim-eval` workflow surface and now holds the remaining claim
  extraction, validation, and eval core
- direct contract coverage:
  `tests/test_claim_extraction_graph.py` now pins the extracted claim-graph seam directly, while
  `tests/test_claim_extraction.py` still verifies the public claim-extraction workflow end to end
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `claim_extraction_graph.py` explicitly in the evidence-and-claims
  container, and `docs/architecture_contract.toml` assigns it to the `claims` layer
- live probe evidence:
  the fresh architecture probe reports `224` code files, `54` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `claim_extraction.py` reduced to `2084` lines from the pre-sequence `2503`-line baseline, new
  module `claim_extraction_graph.py=457`, one fan-out hotspot at `cli_derived=22`, and no Python
  or JS/TS import cycles
- residual risk:
  `claim_extraction.py` still remains a large mixed-owner file because the extraction/query,
  validation, and eval core is still coupled in the public facade
- next routing:
  continue Milestone 6 inside the same umbrella packet on the remaining claim extraction,
  validation, and eval core in `claim_extraction.py`, then `rule_claim_binding.py`,
  `evidence_graph.py`, `package_fact_graph.py`, `applicability_retrieval.py`,
  `applicability_rule_pack.py`, and `applicability_eval.py`; do not advance to Milestone 7 yet
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_claim_extraction.py tests/test_claim_extraction_graph.py -q`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
...
This twelfth Milestone 6 slice closes the remaining applicability validation
artifact/freshness/provenance seam, records the new owner modules in the architecture surfaces, and
keeps the umbrella packet routed inside Milestone 6 instead of pretending the broader
claims/evidence family is closed.
...
- validation-artifact-owner closeout:
  ...
  that previously remained in `applicability_validation.py`
- validation-freshness-owner closeout:
  `src/usfs_r1_ea_sources/applicability_validation_freshness.py` now owns artifact-freshness and
  provenance validation that previously remained in `applicability_validation.py`
- facade-preserving routing:
  `src/usfs_r1_ea_sources/applicability_validation.py` keeps the public
  `validate_applicability_run` entrypoint and is now reduced to a thin validation facade
- direct contract coverage:
  `tests/test_applicability_validation_artifacts.py` now pins the extracted artifact and
  freshness/provenance seams directly, while `tests/test_applicability_validation_checks.py`,
  `tests/test_applicability_decisions.py`, and `tests/test_applicability_adjudication.py` still
  verify the public validation workflow end to end
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `applicability_validation_artifacts.py` and
  `applicability_validation_freshness.py` explicitly in the applicability container, and
  `docs/architecture_contract.toml` assigns both modules to the `applicability` layer
- live probe evidence:
  the fresh architecture probe reports `222` code files, `54` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `applicability_validation.py` reduced to `178` lines from the post-sequence-11 `607`-line
  baseline, new modules `applicability_validation_artifacts.py=173` and
  `applicability_validation_freshness.py=304`, one fan-out hotspot at `cli_derived=22`, and no
  Python or JS/TS import cycles
- residual risk:
  the broader claims/evidence hotspot family remains untouched by this sequence even though the
  applicability validation family is now reduced to explicit owners
- next routing:
  continue Milestone 6 inside the same umbrella packet on the broader claims/evidence owner family,
  starting with `claim_extraction.py`, then `rule_claim_binding.py`, `evidence_graph.py`,
  `package_fact_graph.py`, `applicability_retrieval.py`, `applicability_rule_pack.py`, and
  `applicability_eval.py`; do not advance to Milestone 7 yet
- stale-routing cleanup:
  the immediately following sequence 11 section is now historical for the remaining
  applicability-validation residual note; the current section above supersedes any older packet
  text that still treats `applicability_validation.py` as the owner of artifact-loading,
  artifact-freshness, or provenance validation
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_applicability_decisions.py tests/test_applicability_adjudication.py tests/test_applicability_validation_checks.py tests/test_applicability_validation_artifacts.py -q`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
...
- outcome label:
  `reduced` for Milestone 6 sequence 9; the broader Milestone 6 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- decision-forest-plan seam closeout:
  `src/usfs_r1_ea_sources/applicability_decision_forest_plan.py` now owns Forest Plan component
  predicate evaluation that previously remained in `applicability_decisions.py`
- coverage-owner support closeout:
  `src/usfs_r1_ea_sources/applicability_decision_coverage.py` now also owns candidate-row grouping
  support used by the public decision builder before per-candidate coverage evaluation
- facade-preserving routing:
  `src/usfs_r1_ea_sources/applicability_decisions.py` keeps the public
  `build_applicability_decisions` entrypoint and is now reduced below the `800`-line gate
- direct contract coverage:
  ...
  candidate-row grouping helper and `tests/test_applicability_decisions.py` still verifies the
  public applicability decision behavior end to end
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `applicability_decision_forest_plan.py` explicitly in the
  applicability container, and `docs/architecture_contract.toml` assigns it to the `applicability`
  layer
- live probe evidence:
  the fresh architecture probe reports `213` code files, `55` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `applicability_decisions.py` reduced to `793` lines from the post-sequence-8 `916`-line
  baseline, new module `applicability_decision_forest_plan.py=128`,
  `applicability_decision_coverage.py=227`, one fan-out hotspot at `cli_derived=22`, and no
  Python or JS/TS import cycles
- residual risk:
...
- next routing:
  continue Milestone 6 inside the same umbrella packet on `applicability_validation.py`, then the
  broader claims/evidence owner families; do not advance to Milestone 7 yet
- stale-routing cleanup:
  the immediately following sequence 8 section is now historical for the forest-plan-owner
  residual note; the current section above supersedes any older packet text that still treats
  `applicability_decisions.py` as the owner of Forest Plan component predicate evaluation
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_applicability_decision_arbitration.py tests/test_applicability_decision_coverage.py tests/test_applicability_decision_evidence.py tests/test_applicability_decision_forest_plan.py tests/test_applicability_decision_outputs.py tests/test_applicability_decisions.py -q`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
...
- outcome label:
  `reduced` for Milestone 6 sequence 6; the broader Milestone 6 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- decision-arbitration seam closeout:
  `src/usfs_r1_ea_sources/applicability_decision_arbitration.py` now owns trigger-group
  normalization, rule-contract arbitration, missing-trigger reporting, and arbitration
  summary/effect assembly that previously remained in `applicability_decisions.py`
- facade-preserving routing:
  `src/usfs_r1_ea_sources/applicability_decisions.py` keeps the public
  `build_applicability_decisions` entrypoint plus the remaining evidence-matching and
  forest-plan/coverage predicate core
- direct contract coverage:
  `tests/test_applicability_decision_arbitration.py` now pins the extracted arbitration seam
  directly, while `tests/test_applicability_decisions.py` still verifies the public applicability
  decision behavior end to end
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `applicability_decision_arbitration.py` explicitly in the
  applicability container, and `docs/architecture_contract.toml` assigns it to the `applicability`
  layer
- live probe evidence:
  the fresh architecture probe reports `207` code files, `56` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `applicability_decisions.py` reduced to `1710` lines from the post-sequence-5 `2036`-line
  baseline, new module `applicability_decision_arbitration.py=349`, one fan-out hotspot at
  `cli_derived=22`, and no Python or JS/TS import cycles
- residual risk:
  `applicability_decisions.py` still owns the remaining evidence-matching and forest-plan/coverage
  predicate core, `applicability_validation.py` still remains large, and the claims/evidence
  hotspot family is still untouched by this sequence
- next routing:
  continue Milestone 6 inside the same umbrella packet on the remaining core in
  ...
  owner families; do not advance to Milestone 7 yet
- stale-routing cleanup:
  the immediately following sequence 5 section is now historical for the arbitration-owner
  residual note; the current section above supersedes any older packet text that still treats
  `applicability_decisions.py` as the owner of trigger arbitration or arbitration-summary assembly
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_applicability_decisions.py tests/test_applicability_decision_arbitration.py -q`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
...
This fifth Milestone 6 slice closes the applicability decision-output seam, records the new owner
module in the architecture surfaces, and keeps the umbrella packet routed inside Milestone 6
instead of pretending the broader decision/validation and claims/evidence family is closed.

- outcome label:
  `reduced` for Milestone 6 sequence 5; the broader Milestone 6 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- decision-output seam closeout:
  `src/usfs_r1_ea_sources/applicability_decision_outputs.py` now owns decision partition records,
  provenance payload assembly, summary aggregation, and applicability report rendering that
  previously remained in `applicability_decisions.py`
- facade-preserving routing:
  `src/usfs_r1_ea_sources/applicability_decisions.py` keeps the public
  `build_applicability_decisions` entrypoint plus the remaining predicate/arbitration and
  evidence-matching core, while `src/usfs_r1_ea_sources/applicability_validation.py` now consumes
  the extracted output owner for its adjudication rewrite path
- direct contract coverage:
  `tests/test_applicability_decision_outputs.py` now pins the extracted decision-output seam
  directly, while `tests/test_applicability_decisions.py` still verifies the public applicability
  decision behavior end to end
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `applicability_decision_outputs.py` explicitly in the
  applicability container, and `docs/architecture_contract.toml` assigns it to the `applicability`
  layer
- live probe evidence:
  the fresh architecture probe reports `205` code files, `56` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`,
  `applicability_decisions.py` reduced to `2036` lines from the post-sequence-4 `2374`-line
  baseline, new module `applicability_decision_outputs.py=375`, one fan-out hotspot at
  `cli_derived=22`, and no Python or JS/TS import cycles
- residual risk:
  `applicability_decisions.py` still owns the remaining predicate/arbitration and evidence-matching
  core, `applicability_validation.py` still remains large, and the claims/evidence hotspot family
  is still untouched by this sequence
- next routing:
  continue Milestone 6 inside the same umbrella packet on the remaining core in
  `applicability_decisions.py`, then `applicability_validation.py`, and the broader claims/evidence
  owner families; do not advance to Milestone 7 yet
- stale-routing cleanup:
  the immediately following sequence 4 section is now historical for the applicability decision
  output residual note; the current section above supersedes any older packet text that still
  treats `applicability_decisions.py` as the owner of partition/provenance/report output assembly
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_applicability_decisions.py tests/test_applicability_decision_outputs.py -q`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
...
- outcome label:
  `reduced` for Milestone 6 sequence 3; the broader Milestone 6 family remains active
- routed packet:
  `docs/OVERALL_ARCHITECTURE_REFACTOR_MILESTONE_PLAN.md`
- authority-universe contract seam closeout:
  `src/usfs_r1_ea_sources/applicability_authority_universe_contracts.py` now owns the
  authority-universe snapshot validation checks and summary contract that previously remained in
  `applicability.py`
- facade-preserving routing:
  `src/usfs_r1_ea_sources/applicability.py` keeps the public
  `build_authority_universe_snapshot` entrypoint plus the remaining snapshot orchestration and
  loader/report helpers, but the authority-universe validation/report contract logic is gone
- direct contract coverage:
  `tests/test_applicability_authority_universe_contracts.py` now pins the extracted
  authority-universe validation and summary surface directly, while `tests/test_applicability.py`
  still verifies the public authority-universe snapshot behavior end to end
- architecture closeout:
  `docs/ARCHITECTURE.md` now lists `applicability_authority_universe_contracts.py` explicitly in
  the applicability container, and `docs/architecture_contract.toml` assigns it to the
  `applicability` layer
- live probe evidence:
  the fresh architecture probe reports `201` code files, `56` files above `800`, top hotspot
  `src/usfs_r1_ea_sources/project_sow_package.py` at score `104370`, `applicability.py` reduced to
  `501` lines from the post-sequence-2 `1082`-line baseline, new module
  `applicability_authority_universe_contracts.py=594`, one fan-out hotspot at `cli_derived=22`,
  and no Python or JS/TS import cycles
- residual risk:
  `applicability.py` still owns authority-universe snapshot orchestration and loader/report
  helpers, `applicability_decisions.py` and `applicability_validation.py` still remain large, and
  the claims/evidence hotspot family is still untouched by this sequence
- next routing:
  ...
  Milestone 7 yet
- stale-routing cleanup:
  the immediately following sequence 2 section is now historical for the authority-universe
  validation/summary residual note; the current section above supersedes any older packet text that
  still treats `applicability.py` as the owner for snapshot validation and summary logic
- verification:
  `PYTHONPATH=src uv run --extra dev pytest tests/test_applicability.py tests/test_applicability_authority_family_templates.py tests/test_applicability_candidate_assembly.py tests/test_applicability_authority_universe_contracts.py -q`,
  `PYTHONPATH=src uv run --extra dev pytest tests/test_architecture_contract.py tests/test_architecture_quality.py tests/test_debt_contract.py -q`,
  `PYTHONPATH=src uv run --extra dev ruff check src tests`,
  `PYTHONPATH=src python -m compileall src`,
  `python /Users/chunkstand/.codex/skills/code-architecture-governance/scripts/architecture_probe.py --format markdown --max-file-lines 800 --max-fan-out 20`,
...
Milestone 5 closeout commit `df2bd28`.
...
- routing truth:
  ...
  `237c45d`; Milestone 5 packet closeout is `df2bd28`; and no further
  archived replay or packet-local follow-on is routed through this boundary
- verification:
  milestone-plan lints on the touched packets plus `git diff --check` passed
...
- routing truth:
  the archived full-canonical downstream lane remains green on
  `source-set-732a5a91d31736f8`, Milestone 4 runtime closeout remains
  `237c45d`, and the next routed pass is Milestone 5 docs-only durable
  closeout/routing reset rather than another archived replay repair
- verification:
  milestone-plan lints on the touched packets plus `git diff --check` passed
...
packet further and supersedes the reduced update below when they disagree.

- routed plan:
  `docs/FULL_CANONICAL_DOWNSTREAM_FRESHNESS_REFRESH_MILESTONE_PLAN.md`
- durable extraction progress:
  the active extraction manifest and summary on
  `source-set-cac9c7d02b280825` now record `extracted_count=633`,
  `failed_count=2`,
  `chunk_count=97248`,
  `validation_passed=false`, and
  `failure_counts={"docling_conversion_failed": 1, "pdf_text_fallback_empty": 1}`.
- targeted recovery evidence:
  a targeted external-Docling OCR merge replay plus in-environment
  no-timeout Docling retries on `WILD-ESA-038`, `WILD-ESA-054`, and
  `FPS-241` reduced the blocker set from `59` to `2`.
- exact residual extraction blockers:
  `FPS-005` (`docling_conversion_failed`; a fresh upstream redownload
  reproduces the same invalid xref/pages corruption), and
  `FPS-125` (`pdf_text_fallback_empty`).
- system-tool probes:
  `pdftotext` still returns parse errors on `FPS-005` and zero characters on
  `FPS-125`, while `pdftoppm` can render `FPS-125`. A no-timeout
  in-environment OCR retry on `FPS-125` remained compute-bound for roughly
  `40` minutes and was interrupted without a merged success result. The
  remaining lane is therefore narrowed to upstream PDF replacement/repair
  plus one OCR-heavy raster recovery straggler rather than downloader/catalog
  drift.
- downstream truth:
  ...
  replay is still blocked on the `2` residual extraction failures. The
  current exact downstream red surface remains the same four required
  full-canonical failures recorded below: missing
...
- next routing:
  replace or repair `FPS-005`, recover or replace `FPS-125`, then rerun
  `nepa-knowledge-graph-export`,
  `forest-plan-profile-eval`,
  `forest-plan-component-retrieval-eval`, and
  `promotion-suite` on `source-set-cac9c7d02b280825`.
...
- workbook and parser evidence:
  ...
  hosts so the repaired federal rows stay on the structured-web path.
- scoped replay evidence:
...
- active blocker surface:
  ...
  before the next full-master rerun is now only the `6` residual
  `www.usda.gov` timeout rows:
  `USDA-012`, `USDA-013`, `USDA-009`, `USDA-010`, `USDA-008`, and
  `USDA-011`.
- verification:
  `PYTHONPATH=src python -m usfs_r1_ea_sources source-register-validate --workbook usfs_region1_ea_source_register_FINAL_INGEST_READY_2026.xlsx`
  passed with `issue_count=0`;
...
- aligned boundary:
  the live Milestone 1 checkpoint for this lane is now the immediately
  following `Canonical Source Register Import Completion Milestone 1
  Unsupported-Format Slice Checkpoint` section plus the matching top sections
  in `README.md`, `docs/CURRENT_SYSTEM_STATE.md`, and the routed plan file.
- historical routing note:
  the older `Canonical Source Register Import Completion Milestone 1
  Unsupported-Format Slice Checkpoint` section remains the implementation
  evidence, but the older `Canonical Source Register Import Completion
  Milestone 1 Blocker Repair Alignment Closeout` and
  `Canonical Source Register Import Completion Milestone 1 Blocker Repair
  Slice Checkpoint` sections below are now explicit pre-`cf2d5f6` context
  only. Their `20`-row blocker-routing language no longer governs this lane.
- verification:
  `python /Users/chunkstand/.codex/skills/milestone-plan-writer/scripts/lint_milestone_plan.py --strict docs/CANONICAL_SOURCE_REGISTER_IMPORT_COMPLETION_MILESTONE_PLAN.md`
  passed;
  targeted `rg` routing checks across
...
- aligned boundary:
  the live Milestone 1 checkpoint for this lane is now the immediately
  following `Canonical Source Register Import Completion Milestone 1 Full-Master
  Preflight Replay Checkpoint` section plus the matching top sections in
  `README.md`,
  `docs/CURRENT_SYSTEM_STATE.md`, and
  the routed plan file.
- historical routing note:
  the older `Canonical Source Register Import Completion Milestone 1 Workbook
  Repair Alignment Closeout` and `Canonical Source Register Import Completion
  Milestone 1 Directive Workbook Repair Checkpoint` sections below are now
  explicit historical pre-`86efa46` context only. Their “rerun the fresh
  full-master canonical preflight” next-routing language no longer governs
  this lane.
- verification:
  `python /Users/chunkstand/.codex/skills/milestone-plan-writer/scripts/lint_milestone_plan.py --strict docs/CANONICAL_SOURCE_REGISTER_IMPORT_COMPLETION_MILESTONE_PLAN.md`
  passed;
...
Historical note: this section is pre-`86efa46` alignment context only. The
newer `Canonical Source Register Import Completion Milestone 1 Full-Master
Replay Alignment Closeout` and `Canonical Source Register Import Completion
Milestone 1 Full-Master Preflight Replay Checkpoint` sections above now govern
this lane.
...
- committed baseline truth:
  ...
  `config/phase_eval_direct_eval_v1.json`; and the live aggregate profile-eval predecessor exists
  under `config/region1_forest_plan_profile_eval_coverage_v1.json` plus
  `source_library/evaluations/forest_plan_profile/forest_plan_profile_eval_results.json`.
- current contract roster:
  `config/forest_plan_component_eval_seed.json` remains the default shipped contract for
  `review_id="v1-cg-ecid-compliance-review"` on `source-set-ba8d0feae79501b8`;
  `config/forest_plan_component_evals/` still contains only
  `v1-cg-ecid-source-delta-review.json`, bound to `review_id="v1-cg-ecid-source-delta-review"`
  on `source-set-8a4005c8a083af1a`;
  `config/forest_plan_component_evals/west-reservoir-67436.json` is still absent.
- current result roster:
  `source_library/reviews/v1-cg-ecid-compliance-review/forest_plan_component_eval_results.json`
  exists and passes with `35` cases;
  `source_library/reviews/v1-cg-ecid-source-delta-review/forest_plan_component_eval_results.json`
  exists and passes with `36` cases;
  `source_library/reviews/west-reservoir-67436/forest_plan_component_eval_results.json` is still
  absent.
- South Plateau routing decision:
  South Plateau is explicitly out of scope for the minimum governed review set in this packet.
  The live review on `source-set-ba8d0feae79501b8` has component adjudication plus green
  review-scoped `phase-eval`, but it still lacks both a tracked component-eval contract and
  `forest_plan_component_eval_results.json`. If a later milestone wants South Plateau included, it
  must be added as an explicit typed slot rather than assumed implicitly.
- missing surfaces confirmed:
  there is still no standalone component-retrieval eval producer under live code or config, and
  `src/usfs_r1_ea_sources/forest_plan_component_eval.py` still defaults to
  `config/forest_plan_component_eval_seed.json` when `--eval-file` is omitted.
- verification:
  `git status -sb` confirmed a clean start;
  `ls -1 config/forest_plan_component_evals` confirmed only `v1-cg-ecid-source-delta-review.json`;
  targeted Python inspection confirmed the exact contract/result roster and West Reservoir absence;
  targeted `rg` sweeps across `README.md`, `docs/CURRENT_SYSTEM_STATE.md`, `docs/OUTPUT_SCHEMAS.md`,
  and `docs/SESSION_HANDOFF.md` confirmed there is still no standalone component-retrieval eval
  producer or West Reservoir contract/result surface;
  `python /Users/chunkstand/.codex/skills/milestone-plan-writer/scripts/lint_milestone_plan.py --strict docs/FOREST_PLAN_COMPONENT_EVAL_COVERAGE_MILESTONE_PLAN.md`
  passes; and `git diff --check` passes for this docs-only slice.
- next routing:
  Milestone `0` is reduced and complete. The next executable slice in
  `docs/FOREST_PLAN_COMPONENT_EVAL_COVERAGE_MILESTONE_PLAN.md` is Milestone `1`: add the
  standalone component-retrieval eval producer.
