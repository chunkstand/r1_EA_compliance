...
## Project SOW Capabilities Brief

A generated 4-page Project SOW service capabilities brief now lives at
...
`docs/capabilities/nepa_3d_capabilities_brief.pdf` while focusing on producing scopes of work from
structured proposed-action intake.

The brief was generated by `tools/build_project_sow_capabilities_brief.mjs` after a fresh local
operational gate run to `/tmp/project-sow-capabilities-gate`. Current evidence used in the brief:
`3` proving intakes passed, `0` failed cases, `0` system misses, `0` intake omissions, East Crazies
selected `10` resource scopes across `23` proposed-action resource areas with a `115`-node and
`134`-edge intake evidence graph, and the downstream EA handoff emitted `27` future assembly slots.

Boundary: this is a Project SOW planning and contracting support brief. It does not change package
