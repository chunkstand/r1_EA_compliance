...
A generated 4-page system-level scope-of-work capabilities brief now lives at
...
`docs/capabilities/assets/`. It mirrors the style and page structure of
`docs/capabilities/nepa_3d_capabilities_brief.pdf` while focusing on the system design and
capabilities for producing scopes of work from structured proposed-action intake.

The brief was generated by `tools/build_project_sow_capabilities_brief.mjs` after a fresh local
operational gate run to `/tmp/project-sow-capabilities-gate`. The brief is intentionally general:
it refers to proving intakes, a representative intake graph, the scope library, the operational
gate, and the handoff pattern rather than centering any one project example. Current evidence used
in the brief: `3` proving intakes passed, `0` failed cases, `0` system misses, `0` intake
omissions, a representative package selected `10` resource scopes across `23` proposed-action
resource areas with a `115`-node and `134`-edge intake evidence graph, and the downstream EA handoff
pattern emitted `27` future assembly slots.
