...
`canonical-source-register-active-current-admission`, with `569` admitted and `13` blocked.
This packet stays downstream while the source-truth lane resolves the remaining `12`
FSM/manual-wrapper blockers plus `USFS-026` and re-establishes the truthful canonical target.
