...
from tests.support.catalog_fixtures import _artifact_sha256
from tests.support.catalog_fixtures import _check
from tests.support.catalog_fixtures import _merged_catalog_role_counts
from tests.support.catalog_fixtures import _read_jsonl
from tests.support.catalog_fixtures import _source_delta_catalog_role_counts
from tests.support.catalog_fixtures import _source_delta_primary_plan_source_record_ids
from tests.support.catalog_fixtures import _write_batch_run
from tests.support.catalog_fixtures import _write_download_run
from tests.support.catalog_fixtures import _write_download_run_records
...
class CatalogTests(unittest.TestCase):
    def test_build_review_catalog_writes_engine_artifacts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(len(records), 708)
            self.assertEqual(manifest["source_count"], 708)
            self.assertEqual(manifest["unique_url_count"], 708)
            self.assertEqual(manifest["status_counts"], {"planned": 708})
            self.assertIsNone(manifest["overrides_path"])
            self.assertIsNone(manifest["overrides_sha256"])
            self.assertEqual(result.summary["source_count"], 708)
            ...
            self.assertEqual(source_count, 708)
            self.assertGreater(topic_count, 0)
            self.assertEqual(citation_count, 708)
            self.assertEqual(partition_count, 708)
